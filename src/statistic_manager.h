...
public:
  ...
  /**
   * @brief Analyse all the pulls generated by the MAB before the simulation begins. This is necessary because
   *  it computes the best arm at each timestep
   *
   * @param pulls all the pulls generated by the MAB before the simulation begins
   */
  void analyze_pulls(vector<vector<double>>& pulls); // only if MAB
   ...
   */
  void write_regrets(int sim); // only if MAB
